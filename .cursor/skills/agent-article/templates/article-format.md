# 出力フォーマット仕様

このファイルは `agent-article` スキルが出力する Markdown の正規フォーマットを定義する。
スクリプト (`extract_article.py` / `extract_xcom.py` / `extract_speakerdeck.py`) はこのフォーマットに準拠して出力する。

## 構造

```
---
{YAML frontmatter}
---

# {タイトル}

{本文}
```

## Frontmatter フィールド

```yaml
---
title: "記事タイトル"           # 必須
source_url: "https://..."       # 必須: 元URL
author: "著者名"                # 任意: 取得できない場合は省略
published_at: "YYYY-MM-DD"      # 任意: 取得できない場合は省略
retrieved_at: "YYYY-MM-DDTHH:MM:SS+09:00"  # 必須: 取得日時 ISO 8601
site: "example.com"             # 必須: ドメイン名
content_type: "article"         # 必須: article | tweet | thread | slides
---
```

## content_type 定義

| 値 | 説明 |
|---|---|
| `article` | 一般的なWebページの記事 |
| `tweet` | x.com の単一ツイート |
| `thread` | x.com のスレッド（複数ツイートの連続） |
| `slides` | Speaker Deck などのスライド資料 |

## 本文変換ルール

- 見出し (h1-h6): `#` ~ `######` に変換
- 段落: 空行区切り
- リスト: `-` (箇条書き) / `1.` (番号付き)
- 引用: `>` プレフィックス
- コードブロック: バッククォート3つで囲む
- 画像: 画像リンクは出力しない。エージェントが画像を読み取り内容を自然言語で説明
- リンク: `[text](url)`
- スレッドのツイート区切り: `---` (水平線)
- スライド区切り: `## スライド {n} / {total}`（Speaker Deck のページ番号）

## Speaker Deck の本文

Speaker Deck はスライド画像を主資料、Transcript を OCR 補助資料として読み、文字列の転記ではなく意味構造を Markdown に再構成する。

- 各スライドの主題を `###` 見出しにする
- 説明は自然な文章にする
- 分類・並列項目・手順はリストにする
- 比較やマトリクスは表にする
- フロー・循環・包含・階層は Mermaid または入れ子リストにする
- グラフは軸、系列、主要値、傾向を記述する
- OCR 由来の不自然な空白、改行、読み順を残さない
- 本文と重複する一般的な画像説明は追加しない
- 図から読み取れない情報を推測で補わない

## ファイル名規則

- 出力先: `agent-articles/md/{sanitized_title}.md`
- サニタイズ: `/\:*?"<>|` を除去、空白を `-` に変換
- 最大100文字に切り詰め

## HTML 出力

- Markdown と同じ内容を `agent-articles/html/{sanitized_title}.html` に出力する
- HTML は単独で開ける HTML5 文書とし、frontmatter は `<meta>` 要素、本文は HTML 要素へ変換する
- Markdown の画像リンクは出力せず、Markdown 側で生成した画像説明を本文に反映する
- Markdown の画像説明・翻訳・Speaker Deck の意味変換が完了した後、`render_html.py` で HTML を再生成する

## 画像説明フォーマット

画像リンクは出力しない。エージェントが画像を読み取り、内容を自然言語で説明する:

```
> **[図]** 2024年1月〜12月の月次売上推移を示す折れ線グラフ。1月の100万円から右肩上がりで推移し、12月に400万円に達している。
```

画像のダウンロードに失敗し、alt テキストがある場合:

```
> **[図]** 製品デモ画面
```

画像ダウンロードに失敗し alt も無い場合はプレースホルダーを除去する。

## 設計原則

1. **内容非改変**: 元記事の内容は要約・編集しない
2. **構造保存**: 見出し階層・リスト構造を忠実に再現
3. **LLM最適化**: frontmatter でメタデータを構造的に提供し、本文は余計な装飾を排除
4. **パース容易**: 他のスキルが YAML パースでメタデータを利用可能
5. **画像内容の可読化**: エージェントが画像を視覚的に読み取り説明文に変換し、画像なしでも内容を把握可能にする
