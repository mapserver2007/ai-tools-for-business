...
- **画像OCR**: 画像リンク (`![alt](url)`) は出力せず、代わりに macOS Vision framework で OCR を実行し、読み取った内容を自然言語のテキスト説明に置換する
...
記事内の画像に対して自動的にOCR（文字認識）を実行し、画像リンクの代わりにテキスト説明を出力する。Markdown に画像リンクは残さない。

**処理フロー:**
1. Markdown 内の `![alt](url)` パターンを検出
2. 画像をダウンロード（一時ファイル）
3. macOS Vision framework (`VNRecognizeTextRequest`) で OCR 実行（日本語・英語対応）
4. 画像リンクを OCR 結果のテキスト説明に置換
5. 一時ファイルを削除
...
テキストも alt も無い場合: 画像の痕跡を残さず除去する。

**フォールバック:** `pyobjc-framework-Vision` が未インストールの場合、OCR をスキップし画像リンクも除去する（stderr に警告を出力）。
